...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  const pushScan = (t: number) => {
    ...
    if (materialRef.current) {
      ...
      if (u.uScanStarts) u.uScanStarts.value = buf;
      if (u.uScanCount) u.uScanCount.value = arr.length;
    }
  };
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (material.uniforms.uSkew) material.uniforms.uSkew.value.set(skew.x, skew.y);
      if (material.uniforms.uTilt) material.uniforms.uTilt.value = tiltCurrent.current * tiltScale;
      if (material.uniforms.uYaw) material.uniforms.uYaw.value = THREE.MathUtils.clamp(yawCurrent.current * yawScale, -0.6, 0.6);

      if (material.uniforms.iTime) material.uniforms.iTime.value = now / 1000;
  ...
  useEffect(() => {
    ...
    if (m) {
      const u = m.uniforms;
      if (u.uLineThickness) u.uLineThickness.value = lineThickness;
      if (u.uLinesColor) (u.uLinesColor.value as THREE.Color).copy(srgbColor(linesColor));
      if (u.uScanColor) (u.uScanColor.value as THREE.Color).copy(srgbColor(scanColor));
      if (u.uGridScale) u.uGridScale.value = gridScale;
      if (u.uLineStyle) u.uLineStyle.value = lineStyle === 'dashed' ? 1 : lineStyle === 'dotted' ? 2 : 0;
      if (u.uLineJitter) u.uLineJitter.value = Math.max(0, Math.min(1, lineJitter || 0));
      if (u.uBloomOpacity) u.uBloomOpacity.value = Math.max(0, bloomIntensity);
      if (u.uNoise) u.uNoise.value = Math.max(0, noiseIntensity);
      if (u.uScanGlow) u.uScanGlow.value = scanGlow;
      if (u.uScanOpacity) u.uScanOpacity.value = Math.max(0, Math.min(1, scanOpacity));
      if (u.uScanDirection) u.uScanDirection.value = scanDirection === 'backward' ? 1 : scanDirection === 'pingpong' ? 2 : 0;
      if (u.uScanSoftness) u.uScanSoftness.value = scanSoftness;
      if (u.uPhaseTaper) u.uPhaseTaper.value = scanPhaseTaper;
      if (u.uScanDuration) u.uScanDuration.value = Math.max(0.05, scanDuration);
      if (u.uScanDelay) u.uScanDelay.value = Math.max(0.0, scanDelay);
    }
    if (bloomRef.current) {
        ...
        if (ts - lastDetect >= 33) {
          ...
          try {
            ...
            if (res && res.detection) {
              ...
              const leftCheek = jaw[3] || jaw[2] || jaw[0];
              const rightCheek = jaw[13] || jaw[14] || jaw[jaw.length - 1];
